...
func TestParseProfileDetails(t *testing.T) {
	xml := `<trt:GetProfilesResponse>
	  <trt:Profiles token="MainStream" fixed="true">
	    ...
	    <tt:VideoEncoderConfiguration token="vec"><tt:Name>VEC</tt:Name><tt:Encoding>H265</tt:Encoding><tt:Resolution><tt:Width>2560</tt:Width><tt:Height>1440</tt:Height></tt:Resolution><tt:RateControl><tt:FrameRateLimit>12</tt:FrameRateLimit><tt:EncodingInterval>2</tt:EncodingInterval><tt:BitrateLimit>1536</tt:BitrateLimit></tt:RateControl></tt:VideoEncoderConfiguration>
	  </trt:Profiles>
	</trt:GetProfilesResponse>`
	...
	require.Equal(t, 12, p[0].FrameRateLimit)
	require.Equal(t, 2, p[0].EncodingInterval)
	require.Equal(t, 1536, p[0].BitrateLimit)
